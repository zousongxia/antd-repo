function redPacket(total = 100, count = 10): number[] {
...
function myRedPacket(total = 100, count = 10) {
  const packets = Array(count).fill(1);

  let amount = total - count;

  for (let i = 0; i < count - 1; i++) {
    const radomMoney = Math.floor((Math.random() * amount) / (count - 1 - i));
    packets[i] += radomMoney;
    amount -= radomMoney;
  }
  packets[count - 1] += amount;
  return packets;
}

export { redPacket, myRedPacket };
